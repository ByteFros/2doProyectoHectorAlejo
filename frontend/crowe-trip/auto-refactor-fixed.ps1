# PowerShell script corregido para refactorización masiva de URLs
...
foreach ($file in $files) {
    if (Test-Path $file) {
        ...
        if ($content -notlike "*apiRequest*config/api*") {
            # Determinar la ruta relativa correcta
            $parts = $file -split "\\"
            $depth = $parts.Count - 2  # Restar src y archivo
            $relativePath = ""
        ...
        # Reemplazar URLs hardcodeadas - patrones simples
        $content = $content -replace "fetch\('http://127\.0\.0\.1:8000/api([^']*)'", "apiRequest('`$1'"
        $content = $content -replace 'fetch\("http://127\.0\.0\.1:8000/api([^"]*)"', 'apiRequest("`$1"'
        $content = $content -replace 'fetch\(`http://127\.0\.0\.1:8000/api([^`]*)`', 'apiRequest(`$1`'
        
        # Reemplazar constantes con URLs
        $content = $content -replace "'http://127\.0\.0\.1:8000/api", "'/api"
        $content = $content -replace '"http://127\.0\.0\.1:8000/api', '"/api'
        
        # Limpiar headers redundantes
        $content = $content -replace ',\s*"Content-Type":\s*"application/json"', ''
        $content = $content -replace '"Content-Type":\s*"application/json",\s*', ''
        $content = $content -replace ',\s*credentials:\s*"include"', ''
        $content = $content -replace 'credentials:\s*"include",\s*', ''
...
Write-Host ""
Write-Host "✅ Refactorización completada!" -ForegroundColor Green
